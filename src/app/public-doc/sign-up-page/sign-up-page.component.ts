...
export class SignUpPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.queryParams.subscribe(params => {
      ...
      this.auth$ = this.http.get<any>(this.profileAPIURL + "/auth/signup?login_challenge=" + params["login_challenge"]).pipe(
        map((data: any) => {
          console.log(data)
  ...
  postSignUp(): void {
    const url = this.profileAPIURL + "/auth/signup?login_challenge=" + this.login_challenge + "&_csrf=" + this.csrf
    const body = {
