...
export class ConsentPageComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute,
    ...
    private appService: AppService) {
      this.profileAPIURL = this.appService.settings.profileAPIURL;
  }

  ngOnInit(): void {
    this.route.queryParams.subscribe(params => {
      this.consent_challenge = params["consent_challenge"]
      this.auth$ = this.http.get<any>(this.profileAPIURL + "/auth/consent?consent_challenge=" + this.consent_challenge).pipe(
        map((data: any) => {
  ...
  postConsent(value:string): void {
    this.http.post(this.profileAPIURL + "/auth/consent?consent_challenge=" + this.consent_challenge + "&_csrf=" + this.csrf, {
      _csrf: this.csrf,
